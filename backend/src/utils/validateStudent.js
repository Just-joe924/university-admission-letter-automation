...
export const validateStudentInput = ({
  ...
  // admission_number is not validated here: it is generated by the database,
  // and the controller rejects any value sent by the client.
  if (
    !full_name ||
    !email ||
    !department ||
...
export const validateStudentRecord = (record) => {
  ...
  if (
    ...
    !hasError("application_number") &&
    !IDENTIFIER_PATTERN.test(student.application_number)
  ) {
    addError(
      "application_number",
      "Application number may only contain letters, numbers and / _ . -"
    );
  }

  for (const { key, message } of IMPORT_GENERATED_COLUMNS) {
    if (student[key]) addError(key, message);
  }
